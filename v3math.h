...
/**
 Angle between a and b; no inverse cosine
 */
 ...
 */
bool v1_equals(float a, float b, float tolerance);
...
inline void v3_from_points(float *dst, float *a, float *b) {
    v3_subtract(dst, b, a);
}

inline void v3_add(float *dst, float *a, float *b) {
    dst[0] = a[0] + b[0];
    dst[1] = a[1] + b[1];
    dst[2] = a[2] + b[2];
}

inline void v3_subtract(float *dst, float *a, float *b) {
    dst[0] = a[0] - b[0];
    dst[1] = a[1] - b[1];
    dst[2] = a[2] - b[2];
}

inline float v3_dot_product(float *a, float *b) {
    float result;
...
inline void v3_cross_product(float *dst, float *a, float *b) {
    int x = 0, y = 1, z = 2;
    float dstTmp[3] = {};
...
inline void v3_scale(float *dst, float s) {
    dst[0] *= s;
    dst[1] *= s;
    dst[2] *= s;
}

inline float v3_angle(float *a, float *b) {
    return acosf(v3_angle_quick(a, b));
}

inline float v3_angle_quick(float *a, float *b) {
    return v3_dot_product(a, b) / (v3_length(a) * v3_length(b));
}

inline void v3_reflect(float *dst, float *v, float *n) {
    float nTmp[3] = { n[0], n[1], n[2] };

    float scale = -2 * v3_dot_product(v, nTmp);
    v3_scale(nTmp, scale);
    v3_add(dst, nTmp, v);
}

inline float v3_length(float *a) {
    int x = 0, y = 1, z = 2;

    return sqrtf(a[x]*a[x] + a[y]*a[y] + a[z]*a[z]);
}

inline void v3_normalize(float *dst, float *a) {
    float lengthInverse = 1 / v3_length(a);
...
inline bool v3_equals(float *a, float *b, float tolerance) {
    float dst[3] = {};
...
inline bool v1_equals(float a, float b, float tolerance) {
